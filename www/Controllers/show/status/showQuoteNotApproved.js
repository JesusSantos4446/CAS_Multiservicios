...
async function loadQuotes() {
    ...
    try {
        ...
        allQuotes = quotes
            .filter(quote => quote.ClientId === clientIdFromURL && quote.Status === 3)
            .sort((a, b) => {
...
function displayPage(page) {
    ...
    quotesToDisplay.forEach(({ QuoteName, Price, Date, Id, TypeQuote }) => {
        ...
        let destinationURL = '';
        if (TypeQuote === 1) {
            destinationURL = `./not-approved2.html?clientId=${clientIdFromURL}&quoteId=${Id}`;
        } else if (TypeQuote === 2) {
            destinationURL = `./not-approved3.html?clientId=${clientIdFromURL}&quoteId=${Id}`;
        } else {
