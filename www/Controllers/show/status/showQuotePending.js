...
async function loadQuotes() {
    ...
    try {
        ...
        allQuotes = quotes
            .filter(quote => quote.ClientId === clientIdFromURL && quote.Status === 2)
            .sort((a, b) => {
...
function displayPage(page) {
    ...
    quotesToDisplay.forEach(({ QuoteName, Price, Date, Id, TypeQuote }) => {
        ...
        let destinationURL = '';
        if (TypeQuote === 1) {
            destinationURL = `./pending2.html?clientId=${clientIdFromURL}&quoteId=${Id}`;
        } else if (TypeQuote === 2) {
            destinationURL = `./pending3.html?clientId=${clientIdFromURL}&quoteId=${Id}`;
        } else {
